...
ESP32ベースの汎用開発ボードやM5Stackデバイス をWiFiネットワーク上で動作する**Webファイルサーバー**にするソフトウエアです。Webブラウザを通じて、`LittleFS`または、`SPIFFS`や`SD`のファイルを操作（ファイル一覧表示、アップロード、ダウンロード、削除、画像・音声・テキストファイル等の表示）できるようになります。
